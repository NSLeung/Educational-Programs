...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    if(val < elements[left] || val == elements[left]){
      return 0;
    }
    // C middle =
    while (elements[left] < elements[right] || elements[left] == elements[right]){

      middle = left + (right - left)/2;

      if(elements[middle+1] > elements[middle-1]){return middle + 1;}
      ...
      // std::cout << "inside insertion_idx while loop" <<std::endl;
    }
    std::cout << "before insertion_idx return" <<std::endl;
    return middle;
}
