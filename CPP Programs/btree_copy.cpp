...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if(subroot->is_leaf && subroot->elements.size() <= first_larger_idx){
    return V();
  }
    /* If first_larger_idx is a valid index and the key there is the key we
     * are looking for, we are done. */
     else if(subroot->elements[first_larger_idx].key == key){
     ...
     */
     else if(subroot->is_leaf && subroot->elements[insertion_idx(subroot->elements, subroot->elements[first_larger_idx].value)].key != key){
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    std::cout << "beginning of split_child" <<std::endl;
    //1. Insert new pointer into parent children vector
    parent->children.insert(child_itr, new_right);

    //2. insert offending median into new parent position
    parent->elements.insert(elem_itr, child->elements[child_idx]);

    //3. Copy over elements to right of median to new right node
    new_right->elements.assign(mid_elem_itr + 1, child->elements.end());
    // new_left->elements.erase(mid_child_itr, child->elements.end());
    new_left->elements.assign(child->elements.begin(), mid_elem_itr);


    std::cout << "end of split_child" <<std::endl;
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    if(subroot->is_leaf && subroot->elements[first_larger_idx].key != pair.key){
      //insert into leaf
      ...
      insert(node, pair);
      //check if child became too large; split if necessary
